...
URL params: `?size=120` bigger · `?subs=1` show sub count · `?ends=0` hide end date · `?rate=20` starting sub value · `?admins=name1,name2` extra allowed commanders · `?demo=1` style preview · `?channel=slug&chatroom=ID` other channel · `?color=53fc18` timer hex · `?stroke=000000&strokew=3` outline hex + px (chat commands override params).
...
Overlay params: `?size=120` · `?subs=1` · `?ends=0` · `?demo=1` (color/stroke come from the panel, not the URL).
